    ...
    match instr {
        ...
        Instruction::Value {
            ...
            if args.len() != callee_func.args.len() {
                return Err(InterpError::BadNumArgs(
                    callee_func.args.len(),
                    args.len(),
                ));
            }
            args.iter().zip(callee_func.args.iter()).try_for_each(
                |(arg_name, expected_arg)| {
                    let ty = env.get(arg_name as &str).ok_or_else(|| {
                        InterpError::VarUndefined(arg_name.to_string())
                    })?;
            ...
            callee_func.return_type.as_ref().map_or_else(
                || {
                    Err(InterpError::NonEmptyRetForFunc(
...
fn type_check_func(
    ...
    let mut env: FxHashMap<&str, &Type> = FxHashMap::with_capacity_and_hasher(
        20,
