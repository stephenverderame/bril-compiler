                ...
                funcs: funcs
                    ...
                    .map(|f| {
                        func_map.get(f).copied().ok_or_else(|| {
                            InterpError::FuncNotFound(f.to_string())
                                .add_pos(pos.clone())
                        })
                    })
...
impl BBFunction {
    ...
    fn find_basic_blocks(
        ...
        let args_as_nums = func
            ...
            .iter()
            .map(|a| {
                get_num_from_map(&a.name, &mut num_of_vars, &mut num_var_map)
            })
            .collect();
            ...
            match instr {
                bril_rs::Code::Label { label, pos: _ } => {
                    if !curr_block.instrs.is_empty()
                        || curr_block.label.is_some()
                    {
                        if let Some(old_label) = curr_block.label.as_ref() {
                            label_map
                                .insert(old_label.to_string(), blocks.len());
                        }
            ...
            }) = last_instr
            {
                for l in labels {
                    block.exit.push(
                        *label_map
                            .get(&l)
                            .ok_or(InterpError::MissingLabel(l))?,
                    );
                }
