...
use gc::error::{InterpError, PositionalInterpError};
use gc::{Collector, GenStats, MemStats, MemoryManager};

use fxhash::FxHashMap;

use gc::{Pointer, Value};
...
#[must_use]
fn escape_control_chars(s: &str) -> Option<char> {
    match s {
        "\\0" => Some('\u{0000}'),
        "\\a" => Some('\u{0007}'),
        "\\b" => Some('\u{0008}'),
...
impl Environment {
    ...
    pub fn pop_frame(&mut self) {
        (self.current_pointer, self.current_frame_size) =
            self.stack_pointers.pop().unwrap();
    }
}

// todo: This is basically a copy of the heap implement in brili and we could probably do something smarter.
// This currently isn't that worth it to optimize because most benchmarks do not use the
// memory extension nor do they run for very long.
// You (the reader in the future) may be working with bril programs that you would like to speed up
// that extensively use the bril memory extension. In that case, it would be worth seeing how to
// implement Heap without a map based memory. Maybe try to re-implement malloc for a large Vec<Value>?
struct Heap {
...
impl Default for Heap {
    fn default() -> Self {
        Self {
            memory: FxHashMap::with_capacity_and_hasher(
                20,
...
impl MemoryManager for Heap {
    fn test(&mut self) -> MemStats {
        MemStats {
            gen_stats: vec![GenStats::default()],
            memory_leak: !self.memory.is_empty(),
        }
    }
    ...
    fn write(&mut self, key: &Pointer, val: Value) -> Result<(), InterpError> {
        ...
        let offset: usize = key.offset.try_into().map_err(|_| {
            InterpError::InvalidMemoryAccess(key.base, key.offset)
        })?;
        match self.memory.get_mut(&key.base) {
            ...
            Some(_) | None => {
                Err(InterpError::InvalidMemoryAccess(key.base, key.offset))
            }
        }
    }
    ...
    fn read(&self, key: &Pointer) -> Result<&Value, InterpError> {
        ...
        self.memory
            ...
            .and_then(|val| match val {
                Value::Uninitialized => {
                    Err(InterpError::UsingUninitializedMemory)
                }
                _ => Ok(val),
            })
    }
...
fn make_func_args(
    ...
    args.iter().zip(callee_func.args_as_nums.iter()).for_each(
...
fn execute_value_op<T: std::io::Write, M: MemoryManager>(
    state: &mut State<T, M>,
    op: bril_rs::ValueOps,
    dest: usize,
    args: &[usize],
    ...
    use bril_rs::ValueOps::{
        Add, Alloc, And, Call, Ceq, Cge, Cgt, Char2int, Cle, Clt, Div, Eq,
        Fadd, Fdiv, Feq, Fge, Fgt, Fle, Flt, Fmul, Fsub, Ge, Gt, Id, Int2char,
        ...
        Phi => match last_label {
            ...
            Some(last_label) => {
                let arg = labels
                    ...
                    .ok_or_else(|| {
                        InterpError::PhiMissingLabel(last_label.to_string())
                    })
...
fn execute_effect_op<T: std::io::Write, M: MemoryManager>(
    state: &mut State<T, M>,
    ...
    use bril_rs::EffectOps::{
        Branch, Call, Commit, Free, Guard, Jump, Nop, Print, Return, Speculate,
        Store,
    };
    match op {
        Jump => {
        ...
        Print => {
            ...
            if args.len() == 1 {
                optimized_val_output(
                    &mut state.out,
                    state.env.get(*args.first().unwrap()),
                )?;
                // Add new line
...
fn execute<'a, T: std::io::Write, M: MemoryManager>(
    state: &mut State<'a, T, M>,
    func: &'a BBFunction,
    ...
    loop {
        ...
        for (code, numified_code) in
            curr_instrs.iter().zip(curr_numified_instrs.iter())
        {
            match code {
                Instruction::Constant {
                    ...
                    if const_type == &bril_rs::Type::Float {
                        match value {
                            ...
                            bril_rs::Literal::Int(i) => state.env.set(
    ...
    if args.is_empty() && inputs.is_empty() {
        ...
        args.iter()
            ...
            .try_for_each(|(index, (arg, arg_as_num))| match arg.arg_type {
                ...
                bril_rs::Type::Char => {
                    escape_control_chars(inputs.get(index).unwrap().as_ref())
                        .map_or_else(
...
struct State<'a, T: std::io::Write, M: MemoryManager> {
    prog: &'a BBProgram,
    env: Environment,
    heap: M,
    out: T,
...
impl<'a, T: std::io::Write, M: MemoryManager> State<'a, T, M> {
    const fn new(
        prog: &'a BBProgram,
        env: Environment,
        heap: M,
...
pub fn execute_main<T: std::io::Write, U: std::io::Write>(
    ...
    let tst_results = state.heap.test();
    if tst_results.memory_leak {
        return Err(InterpError::MemLeak)
            .map_err(|e| e.add_pos(main_func.pos.clone()));
    }
    if display_gc {
        eprintln!("{tst_results}");
    }
